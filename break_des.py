    ...
    for row in range(len(S_BOX[sbox_index])):
        for column in range(len(S_BOX[sbox_index][row])):
            if S_BOX[sbox_index][row][column] == output:
                ...
                possible_sbox_input.append(
                    [roww[0], coll[0], coll[1], coll[2], coll[3], roww[1]]
                )
...
def get_possible_sbox_input(E_R, F_R_K):
    ...
    for i in range(len(E_R)):
        ...
        possible_subkeys = [
            xor(E_R[i], possible_input) for possible_input in possible_row_col
        ]
...
def get_possible_subkeys_from_sbox(expansion_result, sbox_output, integer=False):
    ...
    for i in range(len(expansion_result)):
        ...
        # getting row0,col0,col1,col2,col3,row1
        # there are four possible inputs SBOX
        possible_rows_cols = get_possible_sbox_inputs(
            i, expansion_result[i], sbox_output[i]
        )

        possible_subkeys = [
            xor(expansion_result[i], possible_row_col)
            for possible_row_col in possible_rows_cols
        ]
        ...
        if integer:
            possible_k_list[f"K{i+1}"] = [
                convert_to_int(possible_subkey_tested)
                for possible_subkey_tested in possible_subkeys_tested
            ]
        else:
...
def get_integer_possible_subkeys_from_sbox(sbox_input, sbox_output):
    ...
    for i in range(len(sbox_input)):
        ...
        # this possible key just contains of 6 bit of total 48 key bit
        possible_k_list[f"K{i+1}"] = [
            convert_to_int(xor(sbox_input[i], possible_input))
            for possible_input in possible_row_col
        ]
...
def get_intersect_key_diff(possible_k_pt1, possible_k_pt2, possible_k_pt3):
    ...
    for s in range(8):
        for i, j, k in product(list(range(4)), list(range(4)), list(range(4))):
            if (
                possible_k_pt1[s][i] == possible_k_pt2[s][j]
                or possible_k_pt2[s][j] == possible_k_pt3[s][k]
                or possible_k_pt1[s][i] == possible_k_pt3[s][k]
            ):
...
def get_intersect_key_dict_diff(possible_k_pt1, possible_k_pt2, possible_k_pt3):
    ...
    for s in range(8):
        for i, j, k in product(list(range(4)), list(range(4)), list(range(4))):
            if (
                possible_k_pt1[f"K{0+1}"][i] == possible_k_pt2[f"K{0+1}"][j]
                or possible_k_pt2[f"K{0+1}"][j] == possible_k_pt3[f"K{0+1}"][k]
                or possible_k_pt1[f"K{0+1}"][i] == possible_k_pt3[f"K{0+1}"][k]
            ):
...
def calculate_difference(str1, str2):
    return [
        chr(a ^ b) for a, b in zip(string_to_bit_array(str1), string_to_bit_array(str2))
    ]
